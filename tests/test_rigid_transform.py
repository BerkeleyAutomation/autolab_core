...
class RigidTransformTest(unittest.TestCase):
    ...
    def test_bad_inits(self):
        ...
        try:
            T_bad_r = RigidTransform(R)
        except ValueError as e:
            caught_bad_rotation = True
        self.assertTrue(caught_bad_rotation, msg='Failed to catch 3x1 rotation matrix')
        ...
        try:
            ...
            caught_bad_rotation = True
        self.assertTrue(caught_bad_rotation, msg='Failed to catch 3x3x3 rotation matrix')
        ...
        try:
            ...
            caught_bad_rotation = True
        self.assertTrue(caught_bad_rotation, msg='Failed to catch rotation with det != 1')

        # translation with illegal dimensions
        t = np.random.rand(3,3)
        caught_bad_translation = False
        try:
            T_bad_t = RigidTransform(translation=t)
        except ValueError as e:
            caught_bad_translation = True
        self.assertTrue(caught_bad_translation, msg='Failed to catch 3x3 translation')
    ...
    def test_composition(self):
        ...
        self.assertTrue(np.sum(np.abs(T_a_c.matrix - M_a_c)) < 1e-5, msg='Composition gave incorrect transformation') 

        # check frames
        self.assertEqual(T_a_c.from_frame, 'a', msg='Composition has incorrect input frame')
    ...
    def test_point_transformation(self):
        ...
        self.assertTrue(np.sum(np.abs(p_b.vector - x_b)) < 1e-5, msg='Point transformation incorrect: Expected {}, Got {}'.format(x_b, p_b.data)) 
    ...
    def test_bad_transformation(self, num_points=10):
        ...
        self.assertTrue(caught_bad_frame, msg='Failed to catch bad point frame')
        ...
        try:
            ...
            caught_bad_frame = True
        self.assertTrue(caught_bad_frame, msg='Failed to catch bad point cloud frame')
